...
def write_to_strip():
	...
	for index in range(strip_len):
		rgb = [int(255*clamp_value(v)) for v in hls_to_rgb(pixel_matrix[index]['h'] % 1.0, pixel_matrix[index]['l'], pixel_matrix[index]['s'])]
...
led_strip = Strip(APA102.APA102(strip_len, interface='PYDEV', spi_speed=spi_rate, gamma=bibliopixel.gamma.APA102))
...
while True:
	...
	frame += 1
	if frame % 500 == 0:
		print('fps: {}'.format(500 / (time.time() - lasttime)))
		lasttime = time.time()
	try:
...
# 		print(ser_bytes)
	...
	if len(message) == 0 or message[0] != '3':
		continue
	...
	for pi in range(strip_len):
		pixel_matrix[pi]['h'] = accel_data[0]/255.0;
